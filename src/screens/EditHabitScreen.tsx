...
export const EditHabitScreen: React.FC = () => {
  ...
  useEffect(() => {
    if (habit) {
      setInitialValues({
        ...
        category: habit.category as 'health' | 'productivity' | 'mindfulness' | 'fitness' | 'learning' | 'custom',
        color: habit.color,
      ...
      // If it's a custom category, set the custom category text
      if (!['health', 'productivity', 'mindfulness', 'fitness', 'learning'].includes(habit.category)) {
  ...
  const handleSubmit = async (values: FormValues) => {
    try {
      ...
      await updateExistingHabit(habitId, habitData);
      navigation.goBack();
    } catch (error) {
